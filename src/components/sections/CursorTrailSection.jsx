...
export default function CursorTrailSection() {
  useEffect(() => {
    ...
    const movers = [...dots].map((dot, index) => ({
      xTo: gsap.quickTo(dot, "x", {
        duration: 0.2 + index * 0.08,
        ease: "power3.out",
      }),
      yTo: gsap.quickTo(dot, "y", {
        duration: 0.2 + index * 0.08,
        ease: "power3.out",
      }),
    }));
  ...
  return (
    <section className="max-w-[1600px] mx-auto px-8">
      <div className="bg-white  p-20 shadow-xl border border-sky-100">
        <h2 className="text-3xl font-memo text-left text-slate-800">Cursor Trail</h2>
        ...
        {[...Array(12)].map((_, i) => (
          <div
            ...
            style={{
              ...
              height: `${18 - i * 1.5}px`,
              opacity: 0.8 - i * 0.08,
              background: "linear-gradient(to right,#38bdf8,#f9a8d4)",
              borderRadius: "999px",
